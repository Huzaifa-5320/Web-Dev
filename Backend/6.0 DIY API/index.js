...
app.get("/filter", (req, res) => {
  ...
  // Filter all jokes that match the type (case-insensitive)
  const matchingJokes = jokes.filter(
    j => j.jokeType.toLowerCase() === jokeType.toLowerCase()
  );

  if (matchingJokes.length > 0) {
    res.json(matchingJokes);
  } else {
    res.status(404).json({ error: "No jokes found for that type" });
  }
});
